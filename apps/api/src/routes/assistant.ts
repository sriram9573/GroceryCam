...
assistantRouter.post('/parse-intent', async (req: Request, res: Response) => {
    try {
        ...
        Example 1: "Add 3 apples" -> { intent: "ADD_ITEM", item: { name: "apple", quantity: 3, unit: "count" } }
        Example 2: "Cook chicken" -> { intent: "ASK_CLARIFICATION", question: "Do you have a specific chicken dish in mind?" }
        Example 3: "Butter Chicken" -> { intent: "GENERATE_RECIPES", query: "Butter Chicken" }
        Example 4: (Context: "Cook chicken") "No preference" -> { intent: "GENERATE_RECIPES", query: "Chicken" }
        `;
